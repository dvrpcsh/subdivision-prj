...
# 💡 [개선] 5. 배포 후 애플리케이션 상태 확인
# 컨테이너가 켜지는 데 약간의 시간이 걸릴 수 있으므로 5초간 대기합니다.
echo ">> 5초 후 애플리케이션 상태 확인..."
sleep 5

# `docker ps`로 실행 중인 컨테이너 목록을 확인하고,
# `grep subdivision-app`으로 우리 앱 컨테이너가 있는지 필터링합니다.
if [ -z "$(docker ps | grep subdivision-app)" ]; then
    echo ">> 오류: subdivision-app 컨테이너가 실행되지 않았습니다."
    # 컨테이너가 켜지지 않았다면, 상세한 로그를 출력하여 원인 파악을 돕습니다.
